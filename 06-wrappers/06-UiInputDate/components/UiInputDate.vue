<template>
  <ui-input v-model="inputValue" :type="type" @input="changeValue">
...
export default defineComponent({
  ...
  computed: {
    inputValue(): string {
      if (!Number.isInteger(this.modelValue)) return '';
      ...
      switch (this.type) {
        ...
        case 'time':
          if (this.$attrs['step'] && (this.$attrs['step'] as number) % 60 !== 0) return dateTime.format('HH:mm:ss');
  ...
  methods: {
    changeValue(event: Event): void {
      const ms = (event.target as HTMLInputElement).valueAsNumber;
